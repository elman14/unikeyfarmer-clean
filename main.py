...
def save_account_now(acc: dict):
    """Incremental save — dipanggil per akun selesai (crash-safe)."""
    if not acc:
        return
    with DONE_LOCK:
        try:
            ...
            if config.ACCOUNTS_FILE.exists():
                try:
                    ...
                    old = []
            config.ACCOUNTS_FILE.parent.mkdir(parents=True, exist_ok=True)
            json.dump(old + [acc], open(config.ACCOUNTS_FILE, "w"), indent=2)
            sync_apikeys_file()
        except Exception as e:
            log.error(f"incremental save gagal: {e}")


def sync_apikeys_file():
    """Sync output/apikeys.txt dari accounts.json (append-only style, dedupe)."""
    try:
        keys = [a["api_key"] for a in json.load(open(config.ACCOUNTS_FILE)) if a.get("api_key")]
        config.APIKEYS_FILE.parent.mkdir(parents=True, exist_ok=True)
        config.APIKEYS_FILE.write_text("\n".join(keys) + "\n")
    except Exception:
        pass
...
def worker(wid: int, proxy: str | None, q: Queue, results: list, lock: threading.Lock, stats: dict):
    ...
    while True:
        ...
        try:
            t0 = time.time()
            acc = UnikeyClient(proxy, lg, on_complete=save_account_now).run()
            dt = time.time() - t0
            with lock:
                results.append(acc)
                if acc["valid"]:
